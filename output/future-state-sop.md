...
### RCM platform Manual Charge Review — Optimized Workflow
...
### Purpose
To provide a standardized, efficient, and compliance-ready process for completing manual charge review when RCM platform’s autonomous coding engine requires clinician input.
...
### Definitions
- **Non-Autonomous Case:** A clinical encounter where RCM platform’s AI engine has determined that human review is required before a charge code can be submitted
...
- **Confidence Score:** RCM platform’s internally calculated certainty level for each suggested modifier, displayed as High / Medium / Review
...
| **2** | Hospitalist | Opens notification and clicks case link | Mobile / Desktop | RCM platform case review screen opens directly — no queue navigation required |
...
| **4** | Hospitalist | Selects the appropriate modifier code from the displayed options. *If none of the suggestions are appropriate, use the “Manual Entry” field and document reason.* | RCM platform | Modifier selection recorded. **High-risk OB flag:** If case is tagged OB-High-Risk, Step 5 is mandatory before proceeding. |
...
| **6** | Hospitalist | Reviews auto-generated audit summary (modifier · diagnosis · clinician ID) and submits | RCM platform | Charge submitted. Submission confirmation displayed inline. API confirmation sent to EMR automatically. |
...
1. If the submission times out, RCM platform will display: *“Submission status unknown — check queue before resubmitting”*
