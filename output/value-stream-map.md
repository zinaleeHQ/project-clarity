...
```
[01] Commure flags case → [02] Notification received → [03] Navigate to queue*
...
```
[01] Commure flags case + surfaces inline: AI summary + top 3 modifier suggestions
...
| Step 05 — External modifier lookup | Top 3 AI-suggested modifiers with confidence scores displayed inline | Commure API boundary (display layer only) |
| Step 07 — Navigate to EMR for secondary dx | Secondary dx pre-populated from EMR via existing API | Commure API boundary, EMR Neutrality |
